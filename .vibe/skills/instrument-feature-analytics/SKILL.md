...
Run the app with `DEBUG_LEVEL=1 uv run uvibe` — every telemetry event is logged right before the API call. If the log line appears, the event will reach the pipeline. Use this to confirm the event fires with the expected properties before deploying.
