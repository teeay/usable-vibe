...
class _LogView(ScrollView, can_focus=True):
    ...
    def render_line(self, y: int) -> Strip:
        ...
        for i, line_text in enumerate(wrapped):
            # Replace any ``style=None`` segment with an empty style: Textual's
            # NO_COLOR Monochrome/NoColor filters read ``segment.style.color`` and
            # raise on a None style. An empty Style emits no codes (identical to
            # None visually) but is non-None, so rendering is unchanged while the
            # filter is safe.
            segments = [
                seg
