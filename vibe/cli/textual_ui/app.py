...
"""
    ...
    async def on_remote_event(self, event: BaseEvent, loading_widget: Any) -> None:
        if self._committer is not None:
            # Native mode: route remote durable events through the same committer
            # used for local turns; the legacy hidden EventHandler tree is
            # bypassed. Live-only side effects (loading status, plan live owner)
            # still apply.
            self._apply_native_live_effects(event)
            await self._apply_native_plan_effects(event)
            if isinstance(event, UserMessageEvent):
                self._committer.commit_remote_user_message(event.content)
            else:
                self._committer.handle_event(event)
            return
        if self.event_handler:
    ...
    async def on_remote_finalize_streaming(self) -> None:
        if self._committer is not None:
            # Flush trailing buffered remote assistant/reasoning content, mirroring
            # the local turn-end flush in _handle_agent_loop_events.
            self._committer.flush()
            return
        if self.event_handler:
            await self.event_handler.finalize_streaming()
    ...
    async def _mount_and_scroll(
        ...
        if self._committer is not None:
            ...
            # (resume/history and remote replay are Phase 6). Anything else is an
    ...
    def action_open_plan_in_editor(self) -> None:
        # Native mode owns plan review locally; the remote path (Phase 6) still
        # tracks the plan file on the EventHandler.
...
def run_textual_ui(
    ...
    with stderr_guard():
        ...
        session_id = app.run(inline=True, inline_no_clear=True)
