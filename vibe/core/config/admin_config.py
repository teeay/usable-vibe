...
# /reload awaits this fetch, so keep it short to avoid blocking on a slow endpoint.
...
async def fetch_managed_config(base_url: str, api_key: str) -> ManagedConfigResult:
    ...
    On any failure (network error, unexpected status, or unparsable body) the
    returned result carries an ``error`` message and no config, so the admin
    layer stays empty and a fetch problem has no impact on the client.
    """
    ...
    if not response.is_success:
        logger.debug("Managed config fetch status=%s url=%s", response.status_code, url)
        return ManagedConfigResult(error=f"HTTP {response.status_code}")
